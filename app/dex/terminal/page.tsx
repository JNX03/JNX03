...
export default function Terminal() {
  ...
  const handleSudoAttempt = (password: string) => {
    if (users[currentUser]?.sudoer && password === users[currentUser]?.password) {
      setIsRoot(true)
      addOutput("Password accepted. You now have root privileges.")
      handleCommand(sudoCommand)
    } else {
      addOutput("Sorry, incorrect password.")
    }
  ...
  const executeScript = async (script: string) => {
    const scriptFile = initialFileSystem[`${currentPath}/${script}`] || initialFileSystem[`/usr/bin/${script}`]
    ...
    switch (cmd) {
      ...
      case "ls":
        ...
        const files = Object.entries(initialFileSystem)
          ...
          .filter(([, info]) => showHidden || !info.hidden)
          .map(([path, info]) => {
            const name = path.split("/").pop() || ""
            if (showLong) {
              const permissions = (info.permission || 0o644).toString(8).padStart(3, "0")
              const owner = info.owner || currentUser
              return `${info.type === "directory" ? "d" : "-"}${permissions} ${owner} ${name}`
            }
            return name
          })
        addOutput(files.join(showLong ? "\n" : "  "))
        break
      ...
      case "cd":
        ...
        if (initialFileSystem[resolvedPath]?.type === "directory") {
          setCurrentPath(resolvedPath)
        } else {
          addOutput(`cd: ${newPath}: No such file or directory`)
        }
        break
      ...
      case "cat":
        const filePath = resolvePath(currentPath, params[0])
        const catFile = initialFileSystem[filePath]

        if (!catFile || catFile.type !== "file") {
          addOutput(`cat: ${params[0]}: No such file or directory`)
          break
        }

        if (!checkPermissions(catFile, currentUser, isRoot)) {
          addOutput(commands.permissionDenied())
          break
        }
      ...
      case "psql":
        ...
        const dbName = params.includes("-d") ? params[params.indexOf("-d") + 1] : "main"
...
`)
      ...
      case "chmod":
        ...
        if (initialFileSystem[chmodFilePath]) {
          initialFileSystem[chmodFilePath].permission = Number.parseInt(mode, 8)
